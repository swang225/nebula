...
def show_results(
    ...
    results_path = osp.join(
        root(), "model", "yg_ar_net_cnn_emb_f3", "saved_models", "train_results.pkl"
    )
    res = read_pickle(results_path)
    plot_results(res, title="ygarNetF3")
...
def evaluate(
    ...
    trained_model_path = osp.join(
        root(), "model", "yg_ar_net_cnn_emb_f3", "saved_models", "model_best.pt"
    )
